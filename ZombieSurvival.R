...
# Initiate the test giving you questions
  ...
  for (i in 1:(nitems)) {
    ...
    rconcerto.tinsert("03-Zomb-03Resp", param=c(user_response=user_response, latency=toString(latency), hero=hero, for_hero=for_hero, item=item.selected$item[i], iorder=i))
	};
